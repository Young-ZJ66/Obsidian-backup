...
var PluginGroup = class {
  ...
  addGroup(group) {
    if (!group.wouldHaveCyclicGroups(this.id)) {
      ...
      new import_obsidian.Notice("无法添加此群组，因为这会创建一个群组激活循环：\n 群组 A→群组 B→群组 A。", 4e3);
    }
    return false;
  }
  removePlugin(plugin) {
...
var FilteredGroupsList = class {
  ...
  render() {
    ...
    this.listEL.createSpan({ text: "过滤:" });
...
var GroupEditPluginsTab = class extends HtmlComponent {
  ...
  generateContainer() {
    this.mainEl = this.parentEl.createDiv();
    this.mainEl.createEl("h5", { text: "插件" });
  }
  generateContent() {
    if (!this.mainEl) {
      return;
  ...
  createFilterSection(parentEl) {
    ...
    new import_obsidian7.Setting(filterSection).setName("搜索").addText((txt) => {
      txt.setPlaceholder("搜索插件");
      txt.onChange((search) => {
...
var GroupEditGroupsTab = class {
  ...
  generateGroupsSection(parentElement) {
    ...
    groupSection.createEl("h5", { text: "分组" });
    const searchAndList = groupSection.createEl("div");
    new import_obsidian8.Setting(searchAndList).setName("搜索").addText((txt) => {
      txt.setPlaceholder("搜索群组");
      txt.onChange((search) => {
...
var DeviceSelectionModal = class extends import_obsidian9.Modal {
  ...
  onOpen() {
    ...
    contentEl.empty();
    contentEl.createEl("h2", { text: this.headerText });
    contentEl.createEl("h6", { text: "现有设备" });
...
var GroupEditGeneralTab = class {
  ...
  generateGeneralSettingsSection(contentEl) {
    ...
    generalSettingsSection.createEl("h5", { text: "通用" });
    new import_obsidian10.Setting(generalSettingsSection).setName("命令").setDesc("添加命令以启用 / 禁用此组").addToggle((tgl) => {
      tgl.setValue(this.groupToEdit.generateCommands);
      tgl.onChange((value) => this.groupToEdit.generateCommands = value);
    });
    new import_obsidian10.Setting(generalSettingsSection).setName("自动添加").setDesc("自动将新插件添加到这个组").addToggle((tgl) => {
      var _a;
      tgl.setValue((_a = this.groupToEdit.autoAdd) != null ? _a : false);
      tgl.onChange((value) => this.groupToEdit.autoAdd = value);
    });
    const devicesSetting = new import_obsidian10.Setting(generalSettingsSection).setName("设备").setDesc(this.getDevicesDescription()).addButton((btn) => {
  ...
  GenerateStartupSettings(contentEl) {
    ...
    startupParent.createEl("h6", { text: "启动" });
    let delaySetting;
    ...
    new import_obsidian10.Setting(startupParent).setName("开机加载插件").addDropdown((drp) => {
      behaviourElement = drp.selectEl;
      drp.addOption("enable", "启用");
      drp.addOption("disable", "停用");
    ...
    delaySetting = new import_obsidian10.Setting(startupParent).setName("延时").addSlider((slider) => {
...
var CommandManager = class {
  constructor() {
    ...
    this.cnEnablePrefix = "插件组: 启用 ";
    this.cnDisablePrefix = "插件组: 禁用 ";
...
var GroupEditModal = class extends import_obsidian11.Modal {
  ...
  onOpen() {
    ...
    const nameSettingNameEl = new import_obsidian11.Setting(contentEl).addText((txt) => {
      ...
      txt.onChange((val) => {
        ...
        nameSettingNameEl.setText('编辑 "' + this.groupToEdit.name + '"');
      });
    }).nameEl.createEl("h2", {
      text: '编辑"' + this.groupToEdit.name + '"'
    });
  ...
  generateFooter(parentElement) {
    ...
    new import_obsidian11.Setting(footer).addButton((btn) => {
      ...
      btn.setIcon("copy").setTooltip("复制这个组").onClick(() => this.duplicate());
...
var GroupSettings = class extends HtmlComponent {
  ...
  generateContent() {
    ...
    const header = this.mainEl.createEl("h5", { text: "分组" });
    const content = this.mainEl.createDiv();
    if (this.options.collapsible) {
      makeCollapsible(header, content, this.options.startOpened);
    }
    let addBtnEl;
    new import_obsidian12.Setting(content).setName("加入群组").addText((text) => {
      this.groupNameField = text;
      this.groupNameField.setPlaceholder("输入群组名字").setValue(this.newGroupName).onChange((val) => {
  ...
  async addNewGroup() {
    ...
    if (!id) {
      console.error("创建群组失败，请选择不同的名称，因为已有太多同名群组。");
      return;
    }
...
var AdvancedSettings = class extends HtmlComponent {
  ...
  generateContent() {
    ...
    new import_obsidian15.Setting(content).setName("开发日志").addToggle((tgl) => {
    ...
    new import_obsidian15.Setting(content).setName("同步加载").addToggle((tgl) => {
...
var PluginSettings = class extends HtmlComponent {
  ...
  generateContent() {
    ...
    const header = this.mainEl.createEl("h5", { text: "插件" });
  ...
  GeneratePluginsList(parentEl) {
    ...
    refresh.setTooltip("刷新列表以查看对插件和已分配组所做的更改");
  ...
  GenerateFilterSection(parentEl) {
    const filterSection = parentEl.createDiv();
    new import_obsidian19.Setting(filterSection).setName("搜索").addText((txt) => {
      txt.setPlaceholder("搜索插件");
      txt.onChange((search) => {
        this.searchPlugins(search);
      });
...
var PluginGroupSettings = class extends import_obsidian20.PluginSettingTab {
  ...
  generateGeneralSettings(containerEl) {
    ...
    new import_obsidian20.Setting(content).setName("为组生成命令").addToggle((tgl) => {
      var _a;
    ...
    new import_obsidian20.Setting(content).setName("在卸载 / 加载组时的通知").addDropdown((drp) => {
      var _a;
      drp.addOption("none", "无").addOption("short", "短的").addOption("normal", "正常");
    ...
    new import_obsidian20.Setting(content).setName("状态栏菜单").addDropdown((drp) => {
      var _a;
      drp.addOption("None", "无").addOption("Icon", "图标").addOption("Text", "文本");
  ...
  GenerateDeviceList(contentEl) {
    ...
    const CreateNewDevice = async () => {
      ...
      if (Manager.getInstance().devices.contains(newDeviceName)) {
        new import_obsidian20.Notice("设备名称已用于其他设备。");
        return;
      }
    ...
    const header = contentEl.createEl("h4", { text: "设备" });
    const content = contentEl.createDiv();
    makeCollapsible(header, content);
    let deviceAddBtn;
    const deviceNameSetting = new import_obsidian20.Setting(content).setName("新设备");
    ...
    }).setPlaceholder("设备名字").inputEl.onkeydown = async (e) => {
      if (e.key === "Enter") {
    ...
    Manager.getInstance().devices.forEach((device) => {
      ...
      if (getCurrentlyActiveDevice() === device) {
        deviceSetting.setDesc("当前设备").addButton((btn) => {
          btn.setIcon("trash");
